...
bool compress_and_save(const std::shared_ptr<Spec> spec)
{
    ...
    hr = DirectX::LoadFromTGAFile(spec->inputName.c_str(), &meta, *image);
    if (FAILED(hr)) {
        hr = DirectX::LoadFromWICFile(spec->inputName.c_str(), DirectX::WIC_FLAGS_NONE, &meta, *image);
        if (FAILED(hr)) {
            printf(_T("  DirectX::LoadFromXXXFile failed. [%s]\n"), spec->inputName.c_str());
            return false;
        }
    }
